defmodule DBData.ConfigStore do
  @moduledoc """
  Storage backend that owns and manages configuration, synchronized with caudata (~/.db_data/config.db).
  """
  use GenServer
  require Logger
  ...
  def init(opts) do
    name = Keyword.get(opts, :name, __MODULE__)
    path = Keyword.get(opts, :config_path) || DBData.Config.config_path()
  ...
  defp load_json_fallback(tab) do
    json_path = Path.expand("~/.config/dbdata/profiles.json")
  ...
  defp save_config(state) do
    ...
    file_tab = :ets.new(:dbdata_config_file, [:set, :public])
